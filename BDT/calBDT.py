...
def integrate_histogram(hist, bins, a, b):
    """Calculates the integral of a histogram between a and b."""
    total = 0
    for i in range(len(bins) - 1):
        if bins[i] >= a and bins[i + 1] <= b:
            total += hist[i] * (bins[i + 1] - bins[i])
        elif bins[i] < a and bins[i + 1] > a:
            total += hist[i] * (bins[i + 1] - a)
        elif bins[i] < b and bins[i + 1] > b:
            total += hist[i] * (b - bins[i])
    return total

nbins = 60
#load the data
def main():
    with uproot.open(f"Signal_calana.root") as f:
        signal = f['Features'].arrays(library='pd')

    with uproot.open("BKG_calana.root") as f:
    ...
    #param['colsample_bytree'] = 0.1 # fraction of features to train tree on
    ...
    for fold in range(1):
        ...
        sampled_signal = signal_data.sample(n=40000, random_state=42 + fold, replace=False).reset_index(drop=True)
        sampled_background = background_data.sample(n=40000, random_state=42 + fold, replace=False).reset_index(drop=True)
        ...
        sig_hist, sig_bin = np.histogram(predictions[test.get_label().astype(bool)],bins=np.linspace(0,1,nbins),
                density=True);
        bkgd_hist, bkgd_bin = np.histogram(predictions[~(test.get_label().astype(bool))],bins=np.linspace(0,1,nbins),
                density=True);
    ...
    for threshold in thresholds:
        ...
        if (TP + FN) != 0:
            sig_efficiency = TP / (0.5*40000)
        else:
            sig_efficiency = 0


        if (FP + TN) != 0:
            bkgd_efficiency = FP / (0.5*40000)
        else:
            bkgd_efficiency = 0
    ...
    plt.plot(thresholds, bkgd_efficiencies, label=f'Background efficiency: {optimal_bkgd:.4f}', color='b')
