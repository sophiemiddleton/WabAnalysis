...
def main():
    ...
    with uproot.open("BKG_trkana.root") as f:
    ...
    features = features.drop(['isSignal'])#,'D0','Z0'])
    ...
    for fold in range(1):
        ...
        sampled_signal = signal_data.sample(n=28000, random_state=42 + fold, replace=False).reset_index(drop=True)
        sampled_background = background_data.sample(n=28000, random_state=42 + fold, replace=False).reset_index(drop=True)
    ...
    for threshold in thresholds:
        ...
        if (TP + FN) != 0:
            sig_efficiency = TP / (0.5*28000)
        else:
            sig_efficiency = 0


        if (FP + TN) != 0:
            bkgd_efficiency = FP / (0.5*28000)
        else:
            bkgd_efficiency = 0
